...
PROXMOX_CI_DNS
PROXMOX_CI_GATEWAY
PROXMOX_CI_MAC_ADDRESS
PROXMOX_CI_NETWORK_FILTER_ID_NET_NAME
PROXMOX_CI_CLOUDFLARED_TOKEN
PROXMOX_CI_E2E_LXC_PASSWORD
```

`PROXMOX_CI_ADDRESS` is the nested Proxmox IP without CIDR, for example `192.168.50.230`; `PROXMOX_CI_CIDR` is the same address with prefix, for example `192.168.50.230/24`. If `PROXMOX_CI_CIDR` is omitted, the renderer derives it from `PROXMOX_CI_ADDRESS` with `/24` unless `PROXMOX_CI_PREFIX` is set. `PROXMOX_CI_MAC_ADDRESS` pins the nested VM NIC address for DHCP reservations and renders the installer network filter as `ID_NET_NAME_MAC`; when it is omitted, the CI VM uses `02:AC:10:50:00:01`. `PROXMOX_CI_NETWORK_FILTER_ID_NET_NAME` defaults to `ens18` and is only used when no MAC address is set. The workflow runs on a self-hosted runner labeled `proxmox-e2e`, so the nested API can stay LAN-only. `PROXMOX_CI_CLOUDFLARED_TOKEN` is optional when the runner can reach the nested host locally.
